...
drop view if exists vista_materia_lista;
create view vista_materia_lista as (                       
select 
	  m.id,
	  m.nombre,
      m.descripcion,
      am.cantidad as unidades,
      concat(m.cantidad, ' ',m.unidad) as stock,
      sum(aM.costo)/count(m.id) as costo     
      
from MateriaPrima as m inner join stockMateriaPrima as stk on m.id=stk.idMateriaPrima
					   inner join arriboInsumos as a on a.id=stk.idArriboInsumos
					   inner join arriboMateria as aM on aM.idMateriaPrima = m.id
                       inner join Proveedor as p on a.idProveedor = p.id
                       group by m.id
...
drop view if exists vista_materia_lista_id;
create view vista_materia_lista_id as (
select 
	   m.id,
	   m.nombre,
       concat(m.cantidad, ' ',m.unidad) as unidad,
...
from stockMateriaPrima as stk inner join materiaprima as m on stk.idMateriaPrima = m.id
							  inner join arriboMateria as aM on m.id=aM.idMateriaPrima
                              inner join arriboInsumos as a on a.id=aM.idArriboInsumos
                              inner join proveedor as p on a.idProveedor = p.id
);
...
GRANT SELECT ON renegade.vista_materia_lista TO 'administrador'@'localhost';
GRANT SELECT ON renegade.vista_materia_lista_id TO 'administrador'@'localhost';
FLUSH PRIVILEGES;
